...
import type { GeneratorInput, SystemId } from '../types.ts'
...
describe('generateCharacter with all systems active', () => {
  ...
  it('tracks all active systems', () => {
    const character = generateCharacter(makeFullInput())
    const expectedSystemList: SystemId[] = ['attitudinal', 'enneagram', 'mbti', 'socionics', 'instincts']
...
describe('generateCharacter with all systems null', () => {
  it('returns a valid Character with defaults', () => {
    ...
    expect(character.name).toBeTruthy()
    expect(character.title).toBeTruthy()
    expect(character.stats).toEqual({ willpower: 8, intelligence: 8, spirit: 8, vitality: 8 })
...
describe('statBreakdown', () => {
  it('captures AP base stats and source', () => {
    ...
    // VELF: V=1st(14), E=2nd(10), L=3rd(7), F=4th(4)
    expect(character.statBreakdown.base).toEqual({ willpower: 14, intelligence: 7, spirit: 10, vitality: 4 })
  ...
  it('uses Default source and flat 8s when AP is off', () => {
    ...
    expect(character.statBreakdown.base).toEqual({ willpower: 8, intelligence: 8, spirit: 8, vitality: 8 })
  ...
  it('captures enneagram multipliers and source', () => {
    ...
    expect(Object.keys(character.statBreakdown.multipliers).length).toBeGreaterThan(0)
    expect(character.statBreakdown.multiplierSource).toBe(character.archetype.className)
  })
  ...
  it('captures stat overrides', () => {
    ...
    expect(character.statBreakdown.overrides).toEqual({ willpower: 15, intelligence: 12 })
  })
  ...
  it('base is a snapshot, not a shared reference to DEFAULT_STATS', () => {
    ...
    expect(characterA.statBreakdown.base).not.toBe(characterB.statBreakdown.base)
    expect(characterA.statBreakdown.base).toEqual(characterB.statBreakdown.base)
  })
})
