...
import { MBTI_TYPE_LIST, buildAbilityKit } from '../../data/mbti.ts'
import { computeAbilityPower } from '../modifiers.ts'
import type { StatBlock, AbilitySlot, MBTIType } from '../types.ts'
...
const STAT_SCENARIOS: { name: string; stats: StatBlock }[] = [
  { name: 'min', stats: { willpower: 4, intelligence: 4, spirit: 4, vitality: 4 } },
  { name: 'max', stats: { willpower: 20, intelligence: 20, spirit: 20, vitality: 20 } },
  { name: 'typical-AP', stats: { willpower: 14, intelligence: 10, spirit: 7, vitality: 4 } },
]
...
function computeStats(entryList: number[]): { min: number; max: number; mean: number; stdDev: number } {
  const min = Math.min(...entryList)
  const max = Math.max(...entryList)
  const mean = entryList.reduce((sum, v) => sum + v, 0) / entryList.length
  const variance = entryList.reduce((sum, v) => sum + (v - mean) ** 2, 0) / entryList.length
  const stdDev = Math.sqrt(variance)
  return { min, max, mean: Math.round(mean * 100) / 100, stdDev: Math.round(stdDev * 100) / 100 }
}
...
describe('Ability Power Balance Audit', () => {
  ...
  it('every ability produces a positive power value', () => {
    for (const entry of allEntryList) {
      expect(entry.power, `${entry.mbtiType} ${entry.slot} (${entry.cognitiveFunction}) in ${entry.scenario}`).toBeGreaterThan(0)
    }
  })
    ...
    for (const mbtiType of MBTI_TYPE_LIST) {
      for (const scenario of STAT_SCENARIOS) {
        const heroPower = allEntryList.find(
          (e) => e.mbtiType === mbtiType && e.slot === 'hero' && e.scenario === scenario.name,
        )!.power
        const inferiorPower = allEntryList.find(
          (e) => e.mbtiType === mbtiType && e.slot === 'inferior' && e.scenario === scenario.name,
        )!.power
    ...
    for (const mbtiType of MBTI_TYPE_LIST) {
      for (const scenario of STAT_SCENARIOS) {
        const powerBySlot = Object.fromEntries(
          SLOT_LIST.map((slot) => [
            slot,
            allEntryList.find((e) => e.mbtiType === mbtiType && e.slot === slot && e.scenario === scenario.name)!.power,
          ]),
    ...
    for (const mbtiType of MBTI_TYPE_LIST) {
      for (const slot of SLOT_LIST) {
        const minPower = allEntryList.find(
          (e) => e.mbtiType === mbtiType && e.slot === slot && e.scenario === 'min',
        )!.power
        const maxPower = allEntryList.find(
          (e) => e.mbtiType === mbtiType && e.slot === slot && e.scenario === 'max',
        )!.power
  ...
  it('no function produces an outlier (>2 std dev from its slot mean) in any scenario', () => {
    ...
    for (const scenario of STAT_SCENARIOS) {
      for (const slot of SLOT_LIST) {
        const slotEntryList = allEntryList.filter((e) => e.slot === slot && e.scenario === scenario.name)
    ...
    expect(outlierList, `Outliers found:\n${outlierList.join('\n')}`).toHaveLength(0)
  })

  it('each cognitive function appears exactly 2 times per scenario (once per type that uses it)', () => {
    const functionNameSet = new Set(allEntryList.map((e) => e.cognitiveFunction))

    for (const fn of functionNameSet) {
      for (const scenario of STAT_SCENARIOS) {
        const count = allEntryList.filter((e) => e.cognitiveFunction === fn && e.scenario === scenario.name).length
        // Each function appears in exactly 2 of the 16 types (e.g. Ti is hero for INTP and ISTP)
        expect(count, `${fn} in ${scenario.name} should appear in multiple types`).toBeGreaterThanOrEqual(2)
      }
    }
  })
  ...
  it('power values fall within expected bounds per slot', () => {
    ...
    for (const slot of SLOT_LIST) {
      const slotPowerList = allEntryList.filter((e) => e.slot === slot).map((e) => e.power)
