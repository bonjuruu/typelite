...
import { applyStatMultipliers, applyStatOverrides, applyStatChanges, computeAbilityPower, DEFAULT_STATS } from '../modifiers.ts'
import type { StatBlock, Ability, AbilitySlot } from '../types.ts'
...
describe('applyStatMultipliers', () => {
  it('multiplies stats correctly and rounds to integers', () => {
    const base: StatBlock = { willpower: 14, intelligence: 10, spirit: 7, vitality: 4 }
  ...
  it('leaves stats unchanged when multipliers are empty', () => {
    const base: StatBlock = { willpower: 10, intelligence: 10, spirit: 10, vitality: 10 }
  ...
  it('handles zero stats', () => {
    const base: StatBlock = { willpower: 0, intelligence: 0, spirit: 0, vitality: 0 }
  ...
  it('handles max stats (20)', () => {
    const base: StatBlock = { willpower: 20, intelligence: 20, spirit: 20, vitality: 20 }
    const result = applyStatMultipliers(base, { willpower: 1.2, vitality: 1.1 })
...
describe('applyStatOverrides', () => {
  ...
  it('replaces all stats when all are overridden', () => {
    const base: StatBlock = { willpower: 14, intelligence: 10, spirit: 7, vitality: 4 }
    const result = applyStatOverrides(base, { willpower: 1, intelligence: 2, spirit: 3, vitality: 4 })

    expect(result).toEqual({ willpower: 1, intelligence: 2, spirit: 3, vitality: 4 })
  })
})
...
describe('computeAbilityPower', () => {
  it('computes power with formula: basePower * (1 + statValue / 20)', () => {
    const stats: StatBlock = { willpower: 14, intelligence: 10, spirit: 7, vitality: 4 }
  ...
  it('returns basePower when stat is zero', () => {
    const stats: StatBlock = { willpower: 0, intelligence: 0, spirit: 0, vitality: 0 }
  ...
  it('scales correctly at max stat (20)', () => {
    const stats: StatBlock = { willpower: 20, intelligence: 20, spirit: 20, vitality: 20 }
  ...
  it('scales different abilities off their respective stats', () => {
    const stats: StatBlock = { willpower: 14, intelligence: 4, spirit: 10, vitality: 7 }
    const niAbility = makeAbility({ basePower: 12, scalingStat: 'willpower' })
    const tiAbility = makeAbility({ basePower: 8, scalingStat: 'intelligence' })

    expect(computeAbilityPower(niAbility, stats)).toBe(Math.round(12 * (1 + 14 / 20)))
    expect(computeAbilityPower(tiAbility, stats)).toBe(Math.round(8 * (1 + 4 / 20)))
  })
})
