import type { Character, StatName, EnneagramNumber } from "./types/index.ts";
import { getCenter } from "../data/enneagram/index.ts";
import { STAT_LABELS } from "../data/stats.ts";
import { SLOT_LABELS } from "../data/abilities.ts";
...
export function generateSystemInsights(character: Character): string[] {
  ...
  const highestStatEntry = statEntryList.reduce((a, b) =>
    b[1] > a[1] ? b : a,
  );
  const lowestStatEntry = statEntryList.reduce((a, b) => (b[1] < a[1] ? b : a));

  const heroAbility = character.abilities.find((a) => a.slot === "hero");
  const inferiorAbility = character.abilities.find(
    (a) => a.slot === "inferior",
  );
  ...
  if (hasEnneagram && character.archetype.className.includes("[")) {
    const tritypeMatch =
      character.archetype.className.match(/\[(\d)-(\d)-(\d)\]/);
    if (tritypeMatch) {
      const tritypeNumberList = [
        Number(tritypeMatch[1]),
        Number(tritypeMatch[2]),
        Number(tritypeMatch[3]),
      ] as EnneagramNumber[];
  ...
  if (hasAP && hasMBTI && heroAbility) {
    if (heroAbility.scalingStat === highestStatEntry[0]) {
      candidateList.push({
        priority: 9,
        text: `Your ${character.statBreakdown.baseSource} stack puts ${STAT_LABELS[highestStatEntry[0]]} at ${highestStatEntry[1]}, directly amplifying your ${heroAbility.cognitiveFunction}-Hero ability ${heroAbility.name}.`,
      });
    }
  }
  ...
  if (hasEnneagram && hasMBTI && heroAbility) {
    ...
    const isAggressiveClass = classNameMatchesAny(
      character.archetype.className,
      aggressiveClassList,
    );
    const isMethodicalHero = methodicalFunctionList.includes(
      heroAbility.cognitiveFunction,
    );
    const isSupportClass = classNameMatchesAny(
      character.archetype.className,
      supportClassList,
    );
    const isAggressiveHero = aggressiveFunctionList.includes(
      heroAbility.cognitiveFunction,
    );
  ...
  if (hasEnneagram && hasSocionics) {
    ...
    if (
      alignedClassList &&
      classNameMatchesAny(character.archetype.className, alignedClassList)
    ) {
      candidateList.push({
        priority: 6,
  ...
  if (hasInstincts && hasMBTI && heroAbility) {
    ...
    const hasSxFixation =
      character.archetype.instinctPassiveList?.some(
        (p) => p.name === "Fixation",
  ...
  if (hasInstincts) {
    const fixPassiveCount = character.combatBehavior.passives.filter((p) =>
      p.source.includes("fix"),
    ).length;
  ...
  if (hasInstincts && hasEnneagram) {
    ...
    if (
      isFrontline &&
      classNameMatchesAny(character.archetype.className, strategistClassList)
    ) {
      candidateList.push({
        priority: 4,
      ...
      isStrategist &&
      classNameMatchesAny(character.archetype.className, frontlineClassList)
    ) {
      candidateList.push({
        priority: 4,
  ...
  if (hasSocionics && hasEnneagram) {
    const clubClassSynergyMap: Record<
      string,
      { classList: string[]; text: string }
    > = {
      Practical: {
    ...
    if (
      synergy &&
      classNameMatchesAny(character.archetype.className, synergy.classList)
    ) {
      candidateList.push({
        priority: 3,
        text: synergy.text,
