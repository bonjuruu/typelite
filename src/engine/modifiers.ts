import type { StatBlock, StatName, Ability } from './types.ts'
...
export function applyStatMultipliers(
  ...
  return {
    ...
    intelligence: Math.round(baseStats.intelligence * (multipliers.intelligence ?? 1)),
...
export function computeAbilityPower(ability: Ability, stats: StatBlock): number {
  const statValue = stats[ability.scalingStat]
  const scalingFactor = 1 + statValue / 20
