...
import { serializeToUrl, deserializeFromUrl } from '../urlSerializer.ts'
import type { ManualOverrides } from '../../engine/types.ts'
...
function roundTrip(
  ...
  const params = serializeToUrl(enabledSystems, selections, overrides, characterName)
...
describe('urlSerializer round-trip', () => {
  ...
  it('preserves stat boundary values at min (1) and max (20)', () => {
    ...
    expect(result).not.toBeNull()
    expect(result!.overrides.stats).toEqual({ willpower: 1, intelligence: 20, spirit: 1, vitality: 20 })
  })
