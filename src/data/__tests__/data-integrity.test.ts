...
import { ENNEAGRAM_TYPE_LIST, getWings, getCenter, buildArchetype, buildTritypeArchetype, CENTER_TYPES, CENTER_LIST, ENNEAGRAM_INSTINCT_LIST } from '../enneagram.ts'
import { MBTI_TYPE_LIST, buildAbilityKit, getFunctionStack } from '../mbti.ts'
import { SOCIONICS_TYPE_LIST, buildElementAffinity } from '../socionics.ts'
import { INSTINCT_REALM_LIST, buildCombatBehavior, buildTritypeCombatBehavior, getRealmCenter, INSTINCT_CENTER_LIST, REALMS_BY_CENTER } from '../instincts.ts'
import type { EnneagramNumber, StatName } from '../../engine/types.ts'
...
describe('Attitudinal Psyche data integrity', () => {
  ...
  it('each AP type has valid 4-stat mappings summing to 35', () => {
    ...
    for (const apType of AP_TYPE_LIST) {
      ...
      // 14 + 10 + 7 + 4 = 35
      const total = stats.willpower + stats.intelligence + stats.spirit + stats.vitality
  ...
  it('each AP type assigns unique values to each stat', () => {
    for (const apType of AP_TYPE_LIST) {
      const stats = getBaseStats(apType)
      const valueList = [stats.willpower, stats.intelligence, stats.spirit, stats.vitality]
...
describe('Enneagram data integrity', () => {
  ...
  it('wing adjacency is correct for all types', () => {
    const expectedWings: Record<EnneagramNumber, [EnneagramNumber, EnneagramNumber]> = {
      1: [9, 2],
      2: [1, 3],
      3: [2, 4],
      4: [3, 5],
...
describe('MBTI data integrity', () => {
  ...
  it('hero has highest base power, inferior has lowest', () => {
    for (const type of MBTI_TYPE_LIST) {
      const abilityList = buildAbilityKit(type)
      expect(abilityList[0].basePower).toBeGreaterThan(abilityList[3].basePower)
    }
  })
})
  ...
  it('quadra-element mapping is consistent', () => {
    ...
    for (const type of SOCIONICS_TYPE_LIST) {
      ...
      switch (affinity.quadra) {
        case 'Alpha': expect(alphaElementList).toContain(affinity.element); break
        case 'Beta': expect(betaElementList).toContain(affinity.element); break
        case 'Gamma': expect(gammaElementList).toContain(affinity.element); break
        case 'Delta': expect(deltaElementList).toContain(affinity.element); break
      }
    }
  })
})
