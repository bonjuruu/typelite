...
describe("Deep question counts", () => {
  ...
  it("DEEP_QUESTION_COUNT matches actual lengths", () => {
    ...
    expect(DEEP_QUESTION_COUNT.enneagram).toBe(
      DEEP_ENNEAGRAM_QUESTION_LIST.length,
    );
    expect(DEEP_QUESTION_COUNT.mbti).toBe(DEEP_MBTI_QUESTION_LIST.length);
    expect(DEEP_QUESTION_COUNT.socionics).toBe(
      DEEP_SOCIONICS_QUESTION_LIST.length,
    );
...
describe("Deep question ID uniqueness", () => {
  ...
  it('all deep question IDs start with "deep-"', () => {
    for (const question of allDeepQuestionList()) {
      expect(
        question.id.startsWith("deep-"),
        `Question ${question.id} does not start with "deep-"`,
      ).toBe(true);
    }
  });
...
describe("Deep MBTI question validation", () => {
  const validFunctionKeySet = new Set([
    "Ti",
    "Te",
    "Fi",
    "Fe",
  ...
  it("every cognitive function has at least 4 questions where it receives primary weight", () => {
      ...
      for (const option of question.optionList) {
        for (const [key, value] of Object.entries(option.weights)) {
          if (validFunctionKeySet.has(key)) {
            functionMaxWeight[key] = Math.max(
              functionMaxWeight[key] ?? 0,
              value,
            );
          }
        }
    ...
    for (const fn of validFunctionKeySet) {
      expect(
        functionCount[fn],
        `${fn} should be primary in at least 4 questions, got ${functionCount[fn]}`,
...
describe("Deep Socionics question validation", () => {
  it("quadra question weight keys are valid quadra names", () => {
    ...
    const quadraQuestionList = DEEP_SOCIONICS_QUESTION_LIST.filter((q) =>
      q.id.match(/deep-soc-[1-8]$/),
    );
    for (const question of quadraQuestionList) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid quadra key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
  ...
  it("club question weight keys are valid club names", () => {
    ...
    const clubQuestionList = DEEP_SOCIONICS_QUESTION_LIST.filter((q) =>
      q.id.match(/deep-soc-(9|10|11|12)$/),
    );
    for (const question of clubQuestionList) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid club key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
    ...
    for (const question of ieQuestionList) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid IE weight key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
    ...
    for (const question of DEEP_INSTINCT_CENTER_QUESTION_LIST) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid center key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
    ...
    for (const question of getLongInstinctRealmQuestionList("SUR")) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid SUR realm key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
    ...
    for (const question of getLongInstinctRealmQuestionList("INT")) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid INT realm key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
    ...
    for (const question of getLongInstinctRealmQuestionList("PUR")) {
      for (const option of question.optionList) {
        for (const key of Object.keys(option.weights)) {
          expect(
            validKeySet.has(key),
            `Invalid PUR realm key "${key}" in ${question.id}`,
          ).toBe(true);
        }
      }
    }
