import type { EnneagramNumber } from '../../../engine/types.ts'
import type { BuilderSelections } from '../../../hooks/useCharacterGenerator.ts'
import { getWings, getCenter, getEnneagramType, CENTER_TYPES, CENTER_LIST } from '../../../hooks/useCharacterGenerator.ts'
...
export function TritypePicker({ coreType, selections, onUpdateSelection }: TritypePickerProps) {
  ...
  return (
    <div className="space-y-1.5">
      <span className="block text-[10px] font-semibold uppercase text-gray-500">Tritype &mdash; order affects stat influence</span>
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="flex-1 rounded border border-gray-700 bg-gray-800 px-3 py-1.5 text-sm text-gray-400">
          {coreType} &mdash; {getEnneagramType(coreType).className} ({coreCenter})
        </span>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <Select
            ...
            onChange={(value) => {
              const newSecondFix = value ? (Number(value) as EnneagramNumber) : null
              onUpdateSelection('tritypeSecondFix', newSecondFix)
              // Auto-set default wing for the new fix
              if (newSecondFix) {
                onUpdateSelection('tritypeSecondFixWing', getWings(newSecondFix)[0])
              } else {
        ...
        {secondFix && (
          <div className="w-20">
            <Select
              ...
              onChange={(value) => onUpdateSelection('tritypeSecondFixWing', value ? (Number(value) as EnneagramNumber) : null)}
              placeholder="w..."
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1">
          <Select
            ...
            onChange={(value) => {
              const newThirdFix = value ? (Number(value) as EnneagramNumber) : null
              onUpdateSelection('tritypeThirdFix', newThirdFix)
              if (newThirdFix) {
                onUpdateSelection('tritypeThirdFixWing', getWings(newThirdFix)[0])
              } else {
                onUpdateSelection('tritypeThirdFixWing', null)
              }
            }}
        ...
        {selections.tritypeThirdFix && (
          <div className="w-20">
            <Select
              ...
              onChange={(value) => onUpdateSelection('tritypeThirdFixWing', value ? (Number(value) as EnneagramNumber) : null)}
              placeholder="w..."
