import type { EnneagramNumber, EnneagramInstinct } from '../../../engine/types.ts'
...
import {
  ...
  getInstinctLabel,
  getInstinctFullName,
} from '../../../hooks/useCharacterGenerator.ts'
...
export function EnneagramSelector({ selections, onUpdateSelection, onSetEnneagramType }: EnneagramSelectorProps) {
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        <div className="flex-1">
          <Select
            ...
            onChange={(value) => onSetEnneagramType(value ? (Number(value) as EnneagramNumber) : null)}
            placeholder="Type..."
        ...
        {selections.enneagramType && (
          <div className="w-20 sm:w-28">
            <Select
              ...
              onChange={(value) => onUpdateSelection('enneagramWing', value ? (Number(value) as EnneagramNumber) : null)}
              placeholder="Wing..."
      ...
      {selections.enneagramType && (
        <Select
          ...
          onChange={(value) => onUpdateSelection('enneagramInstinct', (value as EnneagramInstinct) || null)}
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            <Toggle
              ...
              onChange={() => onUpdateSelection('instinctStackEnabled', !selections.instinctStackEnabled)}
          ...
          {selections.instinctStackEnabled && (
            <div className="space-y-1.5">
              <span className="block text-[10px] font-semibold uppercase text-gray-500">
                {selections.enneagramInstinct} (dominant) &rarr; pick 2nd (3rd is auto)
              </span>
              <Select
                value={selections.instinctSecond ?? ''}
                onChange={(value) => onUpdateSelection('instinctSecond', (value as EnneagramInstinct) || null)}
                placeholder="2nd instinct..."
                optionList={ENNEAGRAM_INSTINCT_LIST
                  .filter((i) => i !== selections.enneagramInstinct)
                  .map((instinct) => ({
                    value: instinct,
                    label: `${instinct} — ${getInstinctLabel(instinct)} (${getInstinctFullName(instinct)})`,
                  }))}
              ...
              {selections.instinctSecond && (
                <span className="block text-[10px] text-gray-600">
                  3rd (blind spot): {ENNEAGRAM_INSTINCT_LIST.find((i) => i !== selections.enneagramInstinct && i !== selections.instinctSecond)} — {getInstinctLabel(ENNEAGRAM_INSTINCT_LIST.find((i) => i !== selections.enneagramInstinct && i !== selections.instinctSecond)!)}
                </span>
              )}
            </div>
          )}
      ...
      <div className="flex items-center gap-2">
        <Toggle
          ...
          onChange={() => onUpdateSelection('tritypeEnabled', !selections.tritypeEnabled)}
